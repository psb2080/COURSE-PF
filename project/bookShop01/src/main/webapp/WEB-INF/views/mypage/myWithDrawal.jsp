...
// 	init();
 ...
 function fn_delmember(){
	    var _id=$("#_memberInfo.member_id").val();
		
	    $.ajax({
	       type:"post",
	       async:false,  
	       url:"${contextPath}/mypage/delMember.do",
	       dataType:"text",
	       data: {id:_id},
	       success:function (data,textStatus){
	          if(data=='True'){
...
<form action="${contextPath}/mypage/delMember.do" method="post">
		...
		<table>
			<tbody>
				...
				<tr class="dot_line">
					...
					<td>
					  <input name="member_pw" type="password" size="20" value="${memberInfo.member_pw }" disabled/>
					</td>
					<td>
					...
					<td>
					  <c:choose >
					    <c:when test="${memberInfo.member_gender =='101' }" >
					      <input type="radio" name="member_gender" value="102" disabled/>
				...
				<tr class="dot_line">
					...
					<select name="member_birth_d" disabled>
					  ...
					  <input type="radio" name="member_birth_gn" value="2" checked disabled/>양력
						<span style="padding-left:20px"></span> 
						<input type="radio"  name="member_birth_gn" value="1" disabled/>음력
						</c:when>
						<c:otherwise>
						  <input type="radio" name="member_birth_gn" value="2" disabled/>양력
						  <input type="radio"  name="member_birth_gn" value="1" checked  disabled/>음력
						</c:otherwise>
						</c:choose>
					</td>
					<td>
				...
				<tr class="dot_line">
					...
					<td>
					   <select  name="hp1" id="hp1" disabled>
					 ...
					 - <input type="text" name="hp2" size=4 value="${memberInfo.hp2 }" disabled> 
					 - <input type="text"name="hp3"  size=4 value="${memberInfo.hp3 }" disabled><br> <br>
	
				    </td>
	
				</tr>
				<tr class="dot_line">
					<td class="fixed_join" disabled>이메일<br>(e-mail)</td>
					<td>
					   <input type="text" name="email1" size=10 value="${memberInfo.email1 }" disabled/> @ <input type="text" size=10  name="email2" value="${memberInfo.email2 }" disabled /> 
					   <select name="select_email2" onChange=""  title="직접입력" disabled>
					...
					<c:choose> 
					   <c:when test="${memberInfo.emailsts_yn=='true' }">
					     <input type="checkbox" name="emailsts_yn"  value="Y" checked  disabled/> 쇼핑몰에서 발송하는 e-mail을 수신합니다.
						</c:when>
						<c:otherwise>
				...
				<tr class="dot_line">
					...
					<td>
					   <input type="text" id="zipcode" name="zipcode" size=5 value="${memberInfo.zipcode }"  disabled> <a href="javascript:execDaumPostcode()">우편번호검색</a>
					  <br>
					  <p> 
					   지번 주소:<br><input type="text" id="roadAddress"  name="roadAddress" size="50" value="${memberInfo.roadAddress }" disabled><br><br>
					  도로명 주소: <input type="text" id="jibunAddress" name="jibunAddress" size="50" value="${memberInfo.jibunAddress }" disabled><br><br>
					  나머지 주소: <input type="text"  name="namujiAddress" size="50" value="${memberInfo.namujiAddress }" disabled />
					   </p>
					</td>
					<td>
